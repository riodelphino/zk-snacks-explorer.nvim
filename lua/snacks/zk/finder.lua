  ...
  return function(cb)
    ...
    Tree:get_zk(
      ...
      function(node)
        ...
        -- print("get_zk: a node: " .. vim.inspect(node)) -- DEBUG: この node が,parent とかの無い explorer 側？のテーブルになってる！
        -- local parent = node.parent and items[node.parent.file] or nil -- DEBUG: nil 参照エラー
        local parent = node.parent and items[node.parent.path] or nil
        local status = node.status
        if not status and parent and parent.dir_status then
          status = parent.dir_status
        ...
        local zk_note = zk.notes_cache and zk.notes_cache[node.path] or nil -- DEBUG: walk だけじゃなくここでも title 周りをセットしないといけないのか？
